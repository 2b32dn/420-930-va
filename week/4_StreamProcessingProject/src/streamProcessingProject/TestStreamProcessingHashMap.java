...
public class TestStreamProcessingHashMap {
  public static void main(String[] args) {
    ...
    System.out.println("-----Sorting HashMap Collection with respect to keys-----");
    carHashMap.keySet().stream().sorted().forEach(System.out::println);

    System.out.println("-----Sorting HashMap Collection with respect to values BRAND-----");
    carHashMap.values().stream().map(e -> e.toString()).sorted().forEach(v -> System.out.println(v));
    carHashMap.entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.comparing(Car::getBrand)))
